...
class RabbitMQClient:
    ...
    def register_consumer(self, queue_name: str, proto_class: Callable[[], Message], handler: MessageHandler, response_queue: str | None = None):
        """
    ...
    async def _process_incoming_message(self, message: aio_pika.IncomingMessage, queue_name: str):
        ...
        async with message.process():
            try:
                ...
                if response_obj:
                    ...
                    if not reply_to:
                        logger.warning(f"No reply_to and no default response queue for {queue_name}. Message processed but no response sent.")
                        return
    ...
    async def start_consumers(self):
        ...
        for queue_name, (_, _, response_queue) in self.consumers.items():
            ...
            async def callback(msg, q_name=queue_name):

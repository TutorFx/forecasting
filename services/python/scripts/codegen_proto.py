...
from shared_types import JsonObject


class JsonToProto:
  def __init__(self):
    self.type_mapping = {"string": "string", "integer": "int32", "number": "double", "boolean": "bool"}
  ...
  def convert(self, schema: JsonObject, root_name: str, package_name: str) -> str:
  ...
  def _process_field(self, name: str, definition: JsonObject, number: int) -> tuple[list[str], list[str]]:
    ...
    if prop_type == "object":
      ...
      for m in nested_msgs:
        sub_lines.extend("  " + line for line in m.split("\n"))
      nested_msgs = []  # Clear because we consumed them

      sub_lines.extend("  " + line for line in sub_fields)
      sub_lines.append("}")
      ...
      if item_type == "object":
        ...
        sub_lines.extend("  " + line for line in inner_fields)
        sub_lines.append("}")
